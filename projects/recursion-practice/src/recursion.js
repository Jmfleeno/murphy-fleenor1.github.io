...
var arraySum = function(array) {
  //base case
  if (array.flat(0).length === 1) {
    return array.flat(0)[0];
  } else if (array.flat(1).length === 0) {
    return 0;
  }
  //recursion
  return array.flat(1)[0] + arraySum(array.flat(1).slice(1));
};

// 4. Check if a number is even.
var isEven = function(n) {
  //negitive numbers case
  if (n <= 0) {
    //base case
    if (n === 0) {
      return true;
...
// modulo(17,5) // 2
// modulo(22,6) // 4
var modulo = function(x, y, output = 0) {
output = (x/y) * y
};
...
// nthFibo(7); // 13
// nthFibo(3); // 2
var nthFibo = function(n, fiboNum = 1, index = 0) {
  //edge case
  if (n===0){
    return 0
  }
  ...
  //recursion
  return nthFibo(n, fiboNum+=fiboNum, index++)
};
...
var capitalizeFirst = function(array) {
    ...
    //recursion
    output += input[i][0].toUpperCase() + input[i],slice(1);
    return capitalizeWords(input.slice(1), output)
};
...
var letterTally = function(str, obj = {}, index=0) {
  ...
  //recursion case
  obj[str[0]] = index
  return letterTally(str.slice(1), obj, index++ );
};
...
var compress = function(list, output=[]) {
  ...
  //recursion
  if (output.includes(list[0])===false) {
    output += list[0]
  }
...
var minimizeZeroes = function(array, output=[], index=0) {
  //base case
  if (array.length === 0){
    return output;
  }
  //recursion
  if (array[0] !== output[index-1]){
    return (array.slice(1), output, index++);
};
...
var numToText = function(str, output="") {
  ...
  return numToText(str.splice(1), output)
};
